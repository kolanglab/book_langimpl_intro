...
いきなり全部を作ると迷子になります。そこで、**いちばん小さい VM** から始めて、節を追うごとに命令を少しずつ増やしていきましょう。最初は変数も関数もない、**四則演算だけ**をする VM です。`1 + 2 * 3` を計算できれば成功です。
...
| `[:set_local, i]` | てっぺんを `i` 番目のローカル変数に入れる |
| `[:lt]` `[:gt]` `[:eq]` | 上 2 つを比較し、`1`/`0` を積む |
| `[:jump, addr]` | PC を `addr` にする（無条件ジャンプ） |
| `[:jump_if_false, addr]` | てっぺんが `0` なら PC を `addr` にする |

ここで重要なのが **ローカル変数を名前ではなく番号で扱う**ことです。前章のインタプリタは変数を名前（ハッシュのキー）で引いていましたが、VM では「ローカル変数を `0, 1, 2, ...` と並べた配列」を用意し、命令には**番号**を埋め込みます。配列の添字アクセスはハッシュ検索よりずっと速いからです。「名前を番号に置き換える」この下ごしらえこそ、意味解析の章で触れた「変数に番号を振る」作業です。コンパイラがコンパイル時に一度だけ行えば、実行時は番号で一発アクセスできます。
...
class Compiler
  ...
  def compile_expr(node)
    ...
    when :var
      emit(:get_local, local_index(node[1]))   # 名前を番号に変えて読む
...
`local_index` は、変数名が初めて出てきたら新しい番号を割り当て、2 回目以降は同じ番号を返します。これで名前が番号に変わります。

次は**文**です。代入と、`if`・`while` を扱います。`if` 文は、AST インタプリタでは Ruby の `if` で素直に書けました。しかし平坦な命令列には「入れ子」がありません。そこで **ジャンプ命令**で分岐を表現します。`if cond ... else ... end` は、次の形にコンパイルします。
...
問題は、`jump_if_false L_else` を生成する時点では、`L_else` が命令列の何番目になるか**まだ分からない**ことです。then 側をコンパイルし終えて初めて分かります。そこで、いったん飛び先を空けて命令を出し、あとから正しい番地で**埋め戻す**という定石を使います。これを **バックパッチ（backpatching, 後埋め）** と呼びます。`while` ループも同じジャンプの組み合わせで、「先頭で条件を判定し、偽なら脱出、本体の最後で先頭へ戻る」形に展開します。
...
c = Compiler.new
program.each { |s| c.compile_stmt(s) }
vm = VM.new(c.code, 2)
...
コンパイラ側も少しだけ増えます。関数呼び出し式 `[:call, name, args]` は「引数を左から積んでから `call`」に、`return e` 文は「`e` を積んでから `ret`」にコンパイルします。
...
class Compiler
  ...
  #
  # compile_stmt の case に 1 本追加：
  #   when :return
  #     compile_expr(node[1])               # 戻り値を積み…
  #     emit(:ret)                          # 呼び出し元へ戻る
end
```

（式文が `:call` のときは値が `call` の結果として残るので、`else` 節の `emit(:pop)` で捨てる扱いに含めておきます。全体像は最後の「まとめ」で再掲します。）
...
前節までの VM は `@pc`・`@locals` を直接さわっていました。これを**いま実行中のフレーム** `frame = @frames.last` 越しに読み書きするよう、書き換えます。やることは機械的で、`@pc` → `frame.pc`、`@locals` → `frame.locals`、`@code` → `frame.func[:code]` と置き換えるだけです。そのうえで `call`・`ret` の 2 命令を加えます。
...
> ```
>
> 一方 miniruby では、`def` は実行時には**何も起こりません**。コンパイラがソース全体を先に走査して `def` をすべて集め、`@functions` という表に詰め込んでしまうからです。言い換えると、miniruby は「`def` を実行する」のではなく「`def` を事前に回収する」言語なのです。
>
...
コンパイラと VM をつないで、フィボナッチを動かしましょう。各関数を `Compiler` でコンパイルし、`{code:, nparams:, nlocals:}` の形にまとめて関数表にします（トップレベルの文は `main` という関数にまとめます）。この組み立てを行うのが `compile_program` です。

```ruby
def compile_function(params, body)
  c = Compiler.new
  params.each { |p| c.local_index(p) }   # 引数を先頭の番号 0,1,… に割り当てる
  body.each { |s| c.compile_stmt(s) }
  c.emit(:push, 0)                        # 戻り値が無いとき用の保険
  c.emit(:ret)
...
def compile_program(program)
  functions = {}
  program[:defs].each do |name, params, body|   # 各 def をコンパイル
...
| `[:set_local, i]` | てっぺんを `i` 番目のローカル変数に入れる | ローカル変数 |
...
節ごとに足してきた `compile_expr` と `compile_stmt` を 1 つにまとめると、コンパイラの全体はこうなります。
...
class Compiler
  ...
  def compile_expr(node)
    ...
    when :var  then emit(:get_local, local_index(node[1]))
    when :add, :sub, :mul, :div, :lt, :gt, :eq
...
class VM
  ...
  def do_return
    retval = @stack.pop
    @frames.pop
    @stack.push(retval) unless @frames.empty?
  end
end
```
