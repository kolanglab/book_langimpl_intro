...
前章の**手書きパーサ**（おまけ節）と組み合わせて、実際に走らせてみましょう。ここでは Prism を使う経路ではなく、おまけ節で定義した `tokenize`（正規表現ベースの字句解析）と `Parser` クラスを使います。`parse_program` は `Parser` を `def`・`if`・代入などの文も扱えるよう全体に拡張した関数とします。
