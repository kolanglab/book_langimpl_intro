...
この章では、前章までに定義した配列表現の AST を入力に、MiniRuby を完全に動かすインタプリタを Ruby で実装します。**四則演算と画面出力 → 変数 → 関数** という順番でインタプリタを少しずつ育て、各節では新しく扱えるようになるノードを最初に示します。短いコードで本当に動く処理系ができあがるので、ぜひ手元で動かしてください。
