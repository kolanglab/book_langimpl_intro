...
いきなり全部を作ると迷子になります。そこで、次章の VM と同じ順番で、**いちばん小さいインタプリタ**から始め、節を追うごとに扱えるノードを少しずつ増やしていきましょう。最初は変数も関数もない、**四則演算と画面出力だけ**です。`1 + 2 * 3` を計算して表示できれば成功です。
...
数十行で、最初のインタプリタが動きました。ここからノードを足していきます。なお `:print` は、いまは専用ノードで済ませますが、関数の節で `puts` という組み込み**関数**に作り直し、専用ノードとしては姿を消します（次章の VM での `print` 命令と同じ道筋をたどります）。
...
そこで、`eval` に**環境 `env` を引数として渡す**よう改めます。前節の `eval(node)` が `eval(node, env)` になる、という変化です（次章の VM で `@locals` が増えるのと同じ役割です）。
...
`puts` を**組み込み関数**として `eval_call` の中で特別扱いしている点に注目してください。前節まで専用ノードだった `:print` は、ここで役目を終えます。これからは `puts(式)`、すなわち `[:call, "puts", [式]]` という関数呼び出しで画面に出します（次章の VM でも `print` 命令が `puts` 関数に置き換わるのと同じ流れです）。
...
`:print` は四則演算の節で導入しましたが、関数の節で組み込み関数 `puts`（`[:call, "puts", …]`）に置き換わったので、最終的なノードには含まれていません。次章の VM でも `print` 命令が消えるのと同じです。
