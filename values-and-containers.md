...
両者の本質的な違いは**型情報の置き場所**にあります。タグ付き表現ではタグを値と直接隣接して格納するため、どんな間接参照もなしに型を判別できます。ボックス化では型情報はポインタの先のオブジェクト内に収められており、型を知るには必ずポインタをたどる必要があります。目的（値に型を持たせる）は同じでも、型情報の在り処と、それに伴うメモリアクセスのコストが異なるのです。
