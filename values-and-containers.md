...
両者の違いは C などで実装したときに際立ちます。タグ付き表現では `{ タグ, 値 }` という構造体を VM のスタックスロットに直接格納するので、型の判別にヒープへの追加アクセスが不要です。ボックス化では、スタックスロットはポインタだけを保持し、型を知るにはポインタをたどってヒープ上のオブジェクトを参照する必要があります。上の Ruby の例（`[:int, 42]`）は概念を表したもので、Ruby の配列自体はヒープに乗ります。
