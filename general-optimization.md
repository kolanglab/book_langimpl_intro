...
ここまでの最適化は、1 つの計算をいかに**減らすか**という話でした。最後に、視点を変えた高速化を取り上げます ── 計算を減らすのではなく、**複数の CPU コアに分けて同時に進める** **並列化（parallelization）** です。いまどきの CPU はコアをいくつも積んでいますから、1 コアぶんの速度を限界まで磨くより、4 コア・8 コアで手分けしたほうが、はるかに大きく速くなることがあります。

並列化には、粒度の違ういくつかの段階があります。

- **命令レベル**：1 つのループを CPU の SIMD 命令（一度に複数データを処理）に置き換える ── 前述の**自動ベクトル化**がこれです。
- **ループレベル**：ループの繰り返しを複数コアに割り振り、それぞれが別の範囲を担当する。`for i in 0..n` を 4 等分し、4 コアで同時に回す、といった具合です。
- **タスクレベル**：独立した処理のかたまりを、別々のコアで走らせる。

いずれの場合も鍵になるのは、**データ依存の解析**です。並列化してよいのは「同時に走らせても互いの結果を壊さない」と確かめられた部分だけ。ループの各回が前の回の結果に依存していたら（`sum = sum + a[i]` のように）、勝手に分割すると答えが変わってしまいます。ここでもループ最適化と同じく、**意味を保てる範囲を解析で見極める**のが土台です。

そして並列化には、避けられない上限があります。**アムダールの法則（Amdahl's law）** ── プログラムのうち並列化**できない**部分の割合が、いくらコアを増やしても超えられない速度の天井を決める、という法則です[Amdahl, 1967](#cite:amdahl1967)。たとえ 95% を完全に並列化できても、残り 5% が直列なら、コアを無限に積んでも 20 倍止まり。「コアを倍にすれば倍速くなる」とはいかないのです。並列化のコスト（分割・同期・通信）に見合うだけの並列度があるか ── これもまた、典型的な損得勘定です。

> [!NOTE]
> 並列化は「速くする」ための手段ですが、それを支える**仕組み**（スレッド・プロセス・コルーチン）と、つきまとう**危険**（競合状態、同期）は、応用編の[並行制御](concurrency.md)の章で扱いました。本章はあくまで「最適化＝高速化」の視点から並列化を眺めたものです。並列処理そのものに深く踏み込みたい読者は、姉妹編『[並行・並列言語処理系](https://kolanglab.github.io/book_parallel_lang/#cover)』へ。
