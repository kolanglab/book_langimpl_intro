...
実際に解析してみましょう。Ruby のパーサには歴史的に `RubyVM::AbstractSyntaxTree` や `Ripper` もありますが、ここでは前節で「現在の標準パーサ」として挙げた **Prism** を使います。Prism は Ruby 3.3 から標準添付で（古い Ruby でも `gem install prism` で入ります）、処理系本体から切り離してライブラリ単体で呼べるのが利点です。次のコードを `parse_ast.rb` などの名前で保存し、`ruby parse_ast.rb` で実行してみてください。
