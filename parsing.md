...
ソースコードは、処理系にとってはただの文字の並びにすぎません。`"def add(a, b)\n  a + b\nend"` という文字列を、人間は一目で「引数 2 つの関数定義だ」と読み取りますが、処理系が最初に手にしているのは、文字が一列に連なった列だけです。この文字列を、プログラムの構造を表す木（**抽象構文木（AST）**）に変換して、はじめて処理系は中身を扱えるようになります。その変換工程が **構文解析（parsing）** です。この章では、構文解析が何をするのかを理解し、MiniRuby のソースコードを AST にする方法を、**既存のパーサを使う道**と**自分で書く道**の両方から見ていきます。
...
この木が AST です。木の形そのものが「先に `2 * 3` を計算し、その結果に `1` を足す」という計算順序を表しています。**括弧や演算子の優先順位といった「書き方の都合」は、木になった時点ですべて構造に吸収され、消えてなくなります**。これが構文解析の重要なポイントです。後の工程（インタプリタや VM）は、もう優先順位を気にせず、ただ木をたどればよくなります。
...
図にすると、この配列は次のような木に対応します。配列の先頭のシンボルがノードの種類（枝の根）に、残りの要素が子（枝）になっている、という対応です。
...
- **Ruby**：**Prism** が Ruby 3.3 から標準添付され、3.4 以降は CRuby 本体のパーサとしても採用が進んでいます。処理系本体から切り離してライブラリ単体でも使えます。ほかに `RubyVM::AbstractSyntaxTree` や `Ripper` も標準添付です。
...
- **Go**：標準ライブラリの **`go/parser` と `go/ast`** で、Go のソースを解析して AST を得られます。
...
> [!NOTE]
> パーサを**自分で生成したり記述したりする**道（パーサジェネレータ（yacc/bison、ANTLR、Racc）、PEG（Parslet、Treetop）、パーサコンビネータなど）もありますが、これらは文法を自分で書き起こす「パーサの自作」に分類されます。手書きの再帰下降法は次節で、解析手法そのものの体系は後述の姉妹編で扱います。
...
### Ruby のパーサで AST を取り出す

実際に解析してみましょう。Ruby のパーサには歴史的に `RubyVM::AbstractSyntaxTree` や `Ripper` もありますが、ここでは前節の筆頭に挙げた **Prism** を使います。Prism は Ruby 3.3 から標準添付で（古い Ruby でも `gem install prism` で入ります）、処理系本体から切り離してライブラリ単体で呼べるのが利点です。次のコードを `parse_ast.rb` などの名前で保存し、`ruby parse_ast.rb` で実行してみてください。
...
ただし、Prism の AST ノードはそのまま使うには情報が多く、本書独自の `[:add, ...]` 形式とは形が違います。実務でも「既存パーサが返す木を、自分の処理系で扱いやすい形へ**変換（lowering）** する」工程はよくあります。Prism の AST を本書の配列表現へ変換する関数 `convert` の全体は、次のようになります。本書の MiniRuby で扱う範囲のノードはこれひとつで変換でき、後の章の実行例でもこの `convert` をそのまま使います。Ruby 全体の AST を扱えるわけではなく、本書で使う部分集合に限定した変換器である点には注意してください。
...
> [!NOTE]
> `convert` は、receiver も引数もない `call_node`（裸の名前）を変数参照 `[:var, name]` とみなしています。本物の Ruby では、裸の識別子 `x` がローカル変数参照になるかメソッド呼び出しになるかは、そのスコープで先に代入が現れたかどうかで決まります。MiniRuby では簡単のため「裸の名前は変数参照、関数呼び出しは `f(...)` の形」と割り切っているので、この `convert` はあくまで MiniRuby 用の簡略化であって、Ruby 全体の意味を正確に写し取る変換器ではありません。
...
再帰下降法のアイデアはこうです。**文法の規則ひとつひとつを、関数ひとつに対応させる**のです。`add` 規則を解析する関数 `parse_add`、`mul` 規則の `parse_mul`、というように。規則が別の規則を参照していれば、対応する関数を呼び出します。`add` が `mul` を含むなら、`parse_add` は `parse_mul` を呼ぶというように、文法の構造がそのまま関数の呼び出し構造になります。
...
> [!NOTE]
> 上のコードは式（`expr`）の解析に絞っています。文（代入、`if`、`while`）の解析も同じ要領で、`parse_statement` のような関数を足していけば書けます。トップレベル限定の `def` は、文とは別に `parse_toplevel` のような関数で読みます。「`def` を読む → 関数名を読む → `(` 引数列 `)` を読む → `end` まで本体の文を読む」という手順を、そのままコードにします。紙幅の都合で全体は割愛しますが、難しいのは式の優先順位の部分で、文の解析はむしろ素直です。
...
`2 * 3` が内側にまとまり、`+` が外側に来ます。優先順位どおりの木が得られました。これで文字列が木になりました。
...
いずれの道でも、ゴールは同じ、すなわち **後の工程が扱いやすい AST を手に入れること**です。本書ではこの先、上で定義した配列表現の AST を入力として受け取る前提で話を進めます。
