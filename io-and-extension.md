...
ここまでの MiniRuby は、`puts` で数を表示する以外、外の世界とほとんど関わりませんでした。しかし実用的な言語は、ファイルを読み書きし、ネットワークと通信し、キーボード入力を受け取ります。さらに、処理系本体だけではすべての機能をまかなえないので、**後から機能を足すしくみ**も必要です。この章では、入出力（I/O）の位置づけと、処理系を拡張する 3 つの代表的な方法 ── 命令・組み込み・拡張ライブラリ・FFI ── を整理します。
