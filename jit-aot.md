# JIT と AOT のコンパイルと部分評価

ここまでの高速化は、VM の構造を磨いたり、コードを書き換えたりするものでした。本書の最後に、最も野心的な高速化（**機械語を生成して CPU に直接実行させる**）に踏み込みます。AOT と JIT という 2 つのコンパイル戦略、プロファイリングによる賢い JIT、そして「部分評価」という理論を見ていきましょう。これらは現代の高性能処理系（JavaScript エンジン、JVM、近年の Ruby など）の心臓部です[Aycock, 2003](#cite:aycock2003)。
...
最後に、少し理論的な話題、**部分評価（partial evaluation）** を紹介します。これは「コンパイルとは何か」を照らし出す考え方です。
...
ここで、**インタプリタ**を考えてみましょう。インタプリタは「プログラム」と「入力データ」の 2 つを受け取って実行します。このうち「プログラム」のほうだけが分かっているとき、インタプリタを部分評価したら何が得られるでしょうか。
