...
導入に続く本編は、三部構成です。**基礎編**では、MiniRuby を構文解析から AST インタプリタ・仮想マシンまで、**実際に動くところまで**自分の手で作ります。**応用編**では、その土台に文字列・配列・メモリ管理（GC）・例外・クロージャ・並行処理といった「本物の言語の機能」をどう載せるかを、実装の考え方を中心に見渡します。**高速化編**では、できあがった処理系を速くする技法を、VM の磨き方から JIT コンパイルまで段階的にたどります。基礎編は順番に読む前提ですが、応用編の各章は比較的独立して読めます。高速化編は基礎編の VM を出発点にしつつ、ところどころ応用編（特に「値の表現とコンテナ」の章）の知識を使うので、応用編から順に読み進めるのがおすすめです。
